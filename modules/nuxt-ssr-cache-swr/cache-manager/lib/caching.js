...
var caching = function (args) {
  ...
  /**
   * Binds to the underlying store's `peek` function.
   * @function
   * @name peek
   */
  self.peek = self.store.peek.bind(self.store);

  /**
   * Binds to the underlying store's `has` function.
   * @function
   * @name has
   */
  self.has = self.store.has
